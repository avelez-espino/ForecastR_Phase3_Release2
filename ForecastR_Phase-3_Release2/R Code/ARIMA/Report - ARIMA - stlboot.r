...
## windows()
## ARIMA$bias.coefficient.afe.total.age.retro.arima(
##              ARIMA$results.total.age.retro.predictive.performance.arima,
##              str_replace(ARIMA$stockabundance," ","_"))
...
ARIMA$pot9a = pot(paste("Loess Bootstrap"), textProperties(font.size = 20) )
...
ARIMA$tabledata[3,] <- c("Point Forecast",
                   c(as.character(comma(round(ARIMA$results.point.forecast.arima$p))),
                     as.character(comma(sum(round(ARIMA$results.point.forecast.arima$p))))))
...
doc = addParagraph(doc, ARIMA$paragraph, stylename="Normal")
ARIMA$paragraph <- c("   1) Transformation: A Box-Cox transformation is applied to the time series to suppress large fluctuations in its values;",
...
ARIMA$paragraph <- paste("The loess bootstrapping is a time series bootstrapping method introduced by Bergmeir, Hyndman and Benitez in 2014",
...
ARIMA$figurecaption <- paste("Distribution of bootstrapped forecasted values ",
                       ...
                       ARIMA$forecastingyear, 
                       ".",
                       " The dashed red line indicates the position of the point forecast of total abundance on the horizontal axis,",
                       "while the blue segment indicates the 80% forecast interval.", sep=""),
                       " The point forecast for the total abundance was obtained by adding up the point forecasts",
...
ARIMA$paragraph <- paste("MASE was proposed by Hyndman and Koehler (2006) as a generally applicable, scale-free measure of forecast accuracy.",
                       ...
                       # "A scaled error is less than 1 if it arises from a better forecast than the one produced",
                       "by the naive model based on the abundance for the previous year.",
