...
EXPSMOOTH$pot9a = pot(paste("Maximum Entropy Bootstrap"), textProperties(font.size = 20) ) 
...
## tabledata <- M.expsmooth
## tabledata <- subset(M.expsmooth, select=-Model)
...
## tabledata[5 + nrow(M.sub.expsmooth),"Item"] <- "R-squared"

## tabledata[5 + nrow(M.sub.expsmooth),-1] <- paste0(as.character(r.squared.table$R.squared),"%")
...
EXPSMOOTH$paragraph <- paste("Exponential smoothing (ETS) models are a general class of innovations state space models for forecasting a univariate time series.",
                       "The acronym ETS can be thought of as ExponenTial Smoothing, but in effect denotes the error (E), trend (T) and seasonal components (S) which can",
                       "be used to describe the time series to be forecasted.",
...
EXPSMOOTH$paragraph <- paste("Each particular combination of options for the error, trend and seasonal components of a time series gives rise to a specific exponential smoothing model.",
                       "Since the possibilities for each component are Error = {A,M}, Trend = {N,A,Ad,M,Md} and Seasonal = {N,A,M}, in total there exist 2 x 5 x 3 = 30 such",
                       " exponential smoothing models.",
                       "Components designated by the letter N are not present in the time series of interest.  Components designated by the letter A are present",
                       "and are combined with the other components via addition.",
...
EXPSMOOTH$paragraph <- paste("The estimated level, trend or seasonal states of the optimal exponential smoothing model are accompanied by smoothing coefficients,",
                       ...
                       "In the output produced by the ets() function, the smoothing coefficients are denoted by alpha, beta and gamma,",
                       " where alpha is the level smoothing coefficient, beta is the trend smoothing coefficient and gamma is the seasonal smoothing coefficient.",
...
## names(tabledata)[names(tabledata)=="Sum"] <- "Total"
...
for (i in 1:length(EXPSMOOTH$expsmooth.model.fits)) {
     ...
     EXPSMOOTH$figurecaption <- paste("Diagnostic plots for the exponential smoothing model used to forecast the ",
                       EXPSMOOTH$age,
                       " component of the ",
                       # max(results.afe.total.age.retro.expsmooth$CY)+1,
                       EXPSMOOTH$forecastingyear, " ",
                       # " terminal run ",
                       tolower(EXPSMOOTH$stockabundance),
                       " for the ",
...
EXPSMOOTH$paragraph <- paste("The maximum entropy bootstrapping is a time series bootstrapping method introduced by Vinod in 2004 and 2006.",
...
for (i in 1:length(EXPSMOOTH$expsmooth.model.fits)){
    ...
    EXPSMOOTH$figurecaption <- paste("Historical ",
     ...
     EXPSMOOTH$myplot <- EXPSMOOTH$scatterplot.forecasted.values.and.forecast.intervals.individual.ages.expsmooth(
                                  EXPSMOOTH$fits, 
                                  EXPSMOOTH$pointforecasts, 
                                  EXPSMOOTH$intervalforecasts,
                                  i)
...
EXPSMOOTH$myplot <- EXPSMOOTH$scatterplot.forecasted.values.and.forecast.intervals.total.age.expsmooth(
                              EXPSMOOTH$results, 
                              EXPSMOOTH$pointforecasts, 
                              EXPSMOOTH$intervalforecasts)
...
## EXPSMOOTH$myplot <- hist.results.afe.individual.ages.retro.expsmooth(
...
EXPSMOOTH$myplot <- EXPSMOOTH$plot.distribution.bootstrapped.point.forecasts.individual.ages.expsmooth(
                                EXPSMOOTH$PI.individual.ages.expsmooth.sim,
...
## EXPSMOOTH$myplot <- hist.results.afe.total.age.retro.expsmooth(results)

EXPSMOOTH$myplot <- EXPSMOOTH$plot.distribution.bootstrapped.point.forecasts.total.age.expsmooth(
                                      EXPSMOOTH$PI.total.age.expsmooth.sim,
                                      EXPSMOOTH$PI.total.age.expsmooth.no.comma,
                                      EXPSMOOTH$stockabundance)
...
EXPSMOOTH$figurecaption <- paste("Bias coefficient plots constructed from the retrospective forecast errors",
                        "produced by the exponential smoothing models used to forecast ",
                       "specific age components of the ",
                       # max(EXPSMOOTH$results.afe.total.age.retro.expsmooth$CY)+1,
                       EXPSMOOTH$forecastingyear,
                       " ",
...
EXPSMOOTH$myplot <- EXPSMOOTH$scatter.plot.results.afe.individual.ages.retro.expsmooth(
                       results=EXPSMOOTH$results.individual.ages.retro.predictive.performance.expsmooth, EXPSMOOTH$stockabundance)
...
EXPSMOOTH$myplot <- EXPSMOOTH$timeseries.plot.results.afe.total.age.retro.expsmooth(
                      EXPSMOOTH$results.total.age.retro.predictive.performance.expsmooth, EXPSMOOTH$stockabundance)
...
EXPSMOOTH$myplot <- EXPSMOOTH$scatter.plot.results.afe.total.age.retro.expsmooth(
                      EXPSMOOTH$results.total.age.retro.predictive.performance.expsmooth, EXPSMOOTH$stockabundance)
